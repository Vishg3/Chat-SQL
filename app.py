...
radio_opt=["Use SQLITE3 Database - Student.db","Connect to your own MySQL Database"]
...
if not db_uri:
    st.info("Enter the database information and uri")
...
def configure_db(db_uri,mysql_host=None,mysql_user=None,mysql_password=None,mysql_db=None):
    if db_uri==LOCALDB:
        ...
        if not (mysql_host and mysql_user and mysql_password and mysql_db):
            st.error("Provide all MySQL connection details.")
            st.stop()
...
if user_query:
    ...
    with st.chat_message("assistant"):
        streamlit_callack=StreamlitCallbackHandler(st.container())
        response=agent.run(user_query,callbacks=[streamlit_callack])
